...
public class HashITree<V> extends AbstractITree<V>
{
    ...
    {
        V currentNodeValue = contains(target) ? target : findNextNodeToTarget(this.roots, target);
    ...
    @Override public void put(V value)
    {
        ...
        if (nearestNodeValue != null)
        {
            ...
            { if (getSortRule().execute(child, value)) nodeChildren.add(child); }
    ...
    @Override public ITree<V> subtree(V value) throws NullPointerException
    {
        ...
        HashITree<V> subtree = new HashITree<>(getSortRule(), subtreeMap, List.of(value));
        return subtree;
    }
