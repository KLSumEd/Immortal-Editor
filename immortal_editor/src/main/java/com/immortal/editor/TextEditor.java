...
public final class TextEditor extends JFrame 
{
    ...
    private static final int FRAME_WIDTH = 640; // Width of the main editor frame (px)
    private static final int FRAME_HEIGHT = 480; // Height of the main editor frame (px)

    private static final JEditorPane main_editor_pane = new JEditorPane(); // Main editor frame text area

    private static final JMenuBar main_menu_bar = new JMenuBar(); // Main editor menu-bar
    
    private static final JMenu file_submenu = new JMenu("File"); // Main menu-bar 'file' submenu

    private static final JMenuItem file_menuitem_new = new JMenuItem("New"); // 'New' item in 'File' sumbmenu
    private static final JMenuItem file_menuitem_open = new JMenuItem("Open"); // 'Open' item in 'File' sumbmenu
    private static final JMenuItem file_menuitem_save = new JMenuItem("Save"); // 'Save' item in 'File' sumbmenu
    private static final JMenuItem file_menuitem_saveas = new JMenuItem("Save As..."); // 'Save As' item in 'File' sumbmenu
    private static final JMenuItem file_menuitem_quit = new JMenuItem("Quit"); // 'Quit' item in 'File' sumbmenu

    private static final JFileChooser jfc = new JFileChooser(FileSystemView.getFileSystemView().getDefaultDirectory());
    ...
    static 
    {
        ...
        file_menuitem_new.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N, InputEvent.CTRL_DOWN_MASK));
        file_menuitem_open.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, InputEvent.CTRL_DOWN_MASK));
        file_menuitem_save.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.CTRL_DOWN_MASK));
        file_menuitem_saveas.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK));
        file_menuitem_quit.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F4, InputEvent.ALT_DOWN_MASK));

        frame.setJMenuBar(main_menu_bar);
        jfc.setFileFilter(new FileNameExtensionFilter("Plain Text & Immortal Code Files", "txt", "imc"));
    ...
    public void refactorWindow(String new_text, String new_absolute_filepath, String new_filename)
    {
        if (new_text != null) { main_editor_pane.setText(new_text); }
    ...
    public String getAbsoluteFilepath() { return this.absolute_filepath; }
    public void setAbsoluteFilepath(String new_absolute_filepath) { this.absolute_filepath = new_absolute_filepath; }
    ...
    {
        @Override
        public void insertUpdate(DocumentEvent e) { setDocumentModified(); }

        @Override
        public void removeUpdate(DocumentEvent e) { setDocumentModified(); }
    ...
    class MenuBarActionListener implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e) 
        {
            ...
            {
                case "Open" -> { onActionOpen(); }
                case "Save As..."-> { onActionSave(true); }
                case "Save" -> { onActionSave(false); }
                case "New" -> { onActionNew();} 
        ...
        private void onActionOpen()
        {
            ...
            if (isModified() && warnUnsavedChanges() == JOptionPane.CANCEL_OPTION) { return; } 
            ...
            if (returnValue == JFileChooser.APPROVE_OPTION) // If user chose File to Open...
            {
                // Open Selected File
                String new_absolute_filepath = jfc.getSelectedFile().getAbsolutePath(); // Get filepath of selected file
                ...
                } catch (FileNotFoundException ex) { showErrorFileNotFound(file_menuitem_open); return;
                } catch (IOException ex) { showErrorIO(file_menuitem_open); return; }
        ...
        {
            if (!isModified() && !save_as) { return; } // Return if user didn't choose to save and doesn't need to
            ...
            try {
                ...
                if (new_absolute_filepath == null || save_as) 
                {
                    ...
                    int return_value = jfc.showSaveDialog(file_menuitem_save); // Show Dialog and receive input
                    if (return_value != JFileChooser.APPROVE_OPTION) {return;} // Return if the user doesn't wish to save

                    new_absolute_filepath = jfc.getSelectedFile().getAbsolutePath();
                }
                ...
                } catch (FileNotFoundException ex) { showErrorFileNotFound(file_menuitem_save);
                } catch (IOException ex) { showErrorIO(file_menuitem_save); }

            } catch (NullPointerException ex) { showErrorNullPointer(file_menuitem_save); }
        }
        ...
        private int warnUnsavedChanges()
        {
            ...
            int result = JOptionPane.showOptionDialog(main_menu_bar, MESSAGE, TITLE, JOptionPane.YES_NO_CANCEL_OPTION, 
                JOptionPane.WARNING_MESSAGE, null, OPTIONS, OPTIONS[0]);

            switch (result) 
            {
                case JOptionPane.YES_OPTION -> {onActionSave(false);}
